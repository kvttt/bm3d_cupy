"""GPU-accelerated BM3D denoising built on PyTorch."""
...
try:
	...
	raise ImportError(
		"bm3d_torch requires PyTorch, SciPy, and PyWavelets. Install them "
...
def get_wavelet_matrix(n, wavelet, level=None):
	...
	matrix = np.empty((n, n), dtype=I.dtype)
	for i in range(n):
		basis = I[:, i]
		coeffs = pywt.wavedec(basis, wavelet, mode='periodization', level=level)
		matrix[:, i] = np.hstack([np.asarray(coeff) for coeff in coeffs])
	matrix = matrix / np.linalg.norm(matrix, axis=0)
	return torch.as_tensor(matrix, dtype=torch.float32), torch.as_tensor(np.linalg.inv(matrix), dtype=torch.float32)
...
def get_dct_matrix(n, norm='ortho'):
	...
	return torch.as_tensor(matrix, dtype=torch.float32), torch.as_tensor(inverse, dtype=torch.float32)
...
def bm3d_torch(
	...
	if sigma.ndim == 0:
		sigma = torch.full((c,), float(sigma), dtype=torch.float32, device=device)  # (c,)
	elif sigma.numel() == c:
	...
	def _transform_matrices(n, transform_name):  # get transform matrices
		...
		return forward.to(device=device, dtype=torch.float32), inverse.to(device=device, dtype=torch.float32)
	...
	win_1d = torch.kaiser_window(p, beta=2.0, periodic=False, device=device, dtype=torch.float32)  # (p,)
	...
	def _reference_grid():  # get locations of all reference patches
		...
		if ((hp - 1) // ref_stride) * ref_stride != hp - 1:
			ys = torch.cat((ys, torch.as_tensor([hp - 1], dtype=torch.int64, device=device)))
		if ((wp - 1) // ref_stride) * ref_stride != wp - 1:
			xs = torch.cat((xs, torch.as_tensor([wp - 1], dtype=torch.int64, device=device)))
		yy, xx = torch.meshgrid(ys, xs, indexing='ij')
		return yy.reshape(-1), xx.reshape(-1)  # (n_refs,), (n_refs,)
	def _search_offsets():  # get candidate offsets
		offsets = torch.arange(-offset_radius * search_step, offset_radius * search_step + 1, search_step, dtype=torch.int64, device=device)  # (2*offset_radius + 1,)
	...
	# helper arrays
	patch_y = torch.arange(p, dtype=torch.int64, device=device)[None, None, None, :, None]  # (1, 1, 1, p, 1)
	patch_x = torch.arange(p, dtype=torch.int64, device=device)[None, None, None, None, :]  # (1, 1, 1, 1, p)
	patch_c = torch.arange(c, dtype=torch.int64, device=device)[None, None, :, None, None]  # (1, 1, c, 1, 1)
	...
	def _estimate(
		...
		for start in range(0, n_refs, chunk_size):
			...
			dist = torch.sum((cand - ref[:, None, :]) ** 2, dim=2) / patch_norm  # (chunk_size, n_candidates)
			dist = torch.where(valid, dist, torch.full_like(dist, torch.inf))  # (chunk_size, n_candidates)
			nearest = torch.topk(dist, k=group_size, dim=1, largest=False).indices  # select top-`group_size`
			...
			if stage == 'hard':
				threshold = torch.tensor(hard_threshold, dtype=torch.float32, device=device) * sigma_ch[None, None, :, None, None]  # (1, 1, c, 1, 1)
				mask = torch.abs(noisy_coeff) >= threshold  # (chunk_size, group_size, c, p, p)
				coeff = noisy_coeff * mask
				denom = torch.maximum(mask.sum(dim=(1, 3, 4)).to(dtype=torch.float32), torch.tensor(1.0, dtype=torch.float32, device=device))  # (chunk_size, group_size)
			# stage 2 (Wiener filtering)
				...
				noise_power = torch.tensor(wiener_mu2, dtype=torch.float32, device=device) * sigma2_ch[None, None, :, None, None]  # (1, 1, c, 1, 1)
				wiener = pilot_power / (pilot_power + noise_power)  # (chunk_size, group_size, c, p, p)
				# apply gain
				coeff = noisy_coeff * wiener 
				denom = torch.maximum(torch.sum(wiener * wiener, dim=(1, 3, 4)), torch.tensor(1.0, dtype=torch.float32, device=device))  # (chunk_size, group_size)
		...
		# normalize
		out = accum.reshape(h, w, c) / torch.maximum(weight.reshape(h, w, c), torch.tensor(EPS, dtype=torch.float32, device=device))  # (h, w, c)
		return out
	# stage 1 (hard-threshold)
